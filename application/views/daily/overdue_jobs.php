...
<p>This page lists all NSW/ACT Jobs with TBB status that are 15 days or less until deadline.<br/><br/>
</p>
<pre>
<code>SELECT `p`.`property_id`, `p`.`address_1` AS `p_address1`, `p`.`address_2` AS `p_address2`, `p`.`address_3` AS `p_address3`, `p`.`state` AS `p_state`, `p`.`postcode` AS `p_postcode`, `p`.`retest_date`, `p`.`test_date`, `j`.`is_eo`, `j`.`job_type` AS `j_type`, `a`.`agency_id`, `a`.`agency_name`, `j`.`id` AS `id`, DATEDIFF(Date(p.`retest_date`), CURDATE()) AS deadline
FROM `property` AS `p`
...
AND `p`.`retest_date` <= CURDATE() AND (CAST(p.postpone_due_job AS DATE) <= '$today' OR `p`.`postpone_due_job` IS NULL)
