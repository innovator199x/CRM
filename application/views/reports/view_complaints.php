    ...
    <div class="box-typical box-typical-padding">
        <div class="form-row">
            <div class="col-md-2">
                ...
                <button style="float: left" id="add_task_btn"  class="btn btn-inline btn" type="submit">Create Complaint</button>
            </div>

            <!-- <div class="col-md-9">
                <form method=POST action="/reports/view_complaints" class="form-row">
                    <div class="col-10">
                    </div>
                    <div class="col-2">
                        <label>Status</label>
                        <select name="status" class="form-control">                                
                            ...
                            foreach( $complaints_status_sql->result() as $complaints_status_row ){ ?>
                                <option value="<?php echo $complaints_status_row->id; ?>" <?php echo ( $complaints_status_row->id == $this->input->get_post('status') )?'selected':null; ?>><?php echo $complaints_status_row->status; ?></option>
                            <?php
                            }
                            ?>
                        </select>
                    ...
                    <form method=POST action="/reports/view_complaints" class="form-row">
                        ...
                        <div class="col">
                            ...
                            <select name="help_topic" class="form-control">
                                <option value="">---</option>       
                                <?php
                                foreach( $complaints_topic_sql->result() as $comp_topic_row ){ ?>
                                    <option value="<?php echo $comp_topic_row->comp_topic_id; ?>" <?php echo ( $comp_topic_row->comp_topic_id == $this->input->get_post('help_topic') )?'selected':null; ?>><?php echo $comp_topic_row->comp_topic; ?></option>
                                <?php
                                }
                                ?>
                    ...
                    if (count($tasks) > 0) {
                        foreach ($tasks as $row) {
                            ...
                            <tr>
                                ...
                                <td><?php echo ( $row['comp_topic'] > 0 )?$row['comp_topic_details']:'Other'; ?></td>
                                <!-- <td> -->
                                    <?php                             
                                    // ticket priority
                                ...
                                <td>
                                    <a href="/reports/complaints_details/?id=<?php echo $row['comp_id']; ?>">
                        ...
                        ?>
                    <td colspan="8" align="left">No Data</td>
                    <?php
                }
                ?>
...
<div id="add_task_div" class="addproperty formholder">
    <form id="add_task_form" method="post" action="/reports/update_complaints_action_form_submit" enctype="multipart/form-data">
        ...
        <div class="row mb-3">                
            <div class="col-md-6">
                ...
                <select id="comp_topic" name="comp_topic"  class="form-control field_g2" required>
                    ...
                    <?php
                    foreach( $complaints_topic_sql->result() as $comp_topic_row ){ ?>
                        <option value="<?php echo $comp_topic_row->comp_topic_id; ?>"><?php echo $comp_topic_row->comp_topic; ?></option>
                    <?php
                    }
                    ?>
                    <option value="-1">Other</option>
            ...
            <div class="col-md-3">
                <label class="addlabel" for="title"><span class="text-danger">*</span>Assign To</label>
                <input type="text" id="search_managers" class="form-control field_g2" />
                <div id="managers_suggestion_div"></div>
                <div id="subscribed_managers_div" class="mt-2">                                            
                </div>
            </div>
            <div class="col-md-3">
                <label class="addlabel" for="title"><span class="text-danger">*</span>Agency</label>
                <input type="text" id="search_agency" class="form-control field_g2" />
                <div id="agency_suggestion_div"></div>
                <div id="subscribed_agency_div" class="mt-2">                                            
                </div>
            </div>
...
jQuery(document).ready(function () {
    ...
    // $("#assign_to").CreateMultiCheckBox({ width: '230px',
    ...
    jQuery("#add_task_btn").click(function () {
        ...
        var default_btn_txt = 'Create Complaint';
...
<?php } ?>
    ...
    jQuery("#btn_reset").click(function(){
        ...
        jQuery.ajax({
            type: "POST",
            url: "/reports/remove_managers_complaints",
            data: {
                id: 1
            }
        }).done(function (ret) {
            get_managers_complaints();     
            get_agency_complaints();   
        });
    ...
    // suggest agency
    jQuery("#search_agency").keyup(function(){

        var search_agency_dom = jQuery(this);
        var search_agency = search_agency_dom.val();
        ...
        if( search_agency != '' ){
            ...
            jQuery.ajax({
                ...
                jQuery('#load-screen').hide(); 
                jQuery("#agency_suggestion_div").show();
                jQuery("#agency_suggestion_div").html(ret);
    ...
    jQuery("#managers_suggestion_div").on('click','#managers_ul .managers_user_id',function(){
        ...
        if( staff_id > 0 ){
            ...
            jQuery.ajax({
                type: "POST",
                url: "/reports/complaints_temp_managers",
                data: {
                    staff_id: staff_id
                }
            }).done(function (ret) {
    ...
    // subscribe agencies
    jQuery("#agency_suggestion_div").on('click','#managers_ul .managers_user_id',function(){

        var managers_user_id_dom = jQuery(this);
        var agency_id = managers_user_id_dom.attr("data-staff_id");
        // alert(agency_id);
        if( agency_id > 0 ){
            ...
            jQuery.ajax({
                type: "POST",
                url: "/reports/complaints_temp_agency",
                data: {
                    agency_id: agency_id
                }
            }).done(function (ret) {
    ...
    jQuery(document).mouseup(function (e){
        ...
        var container = jQuery("#agency_suggestion_div");
        if (!container.is(e.target) // if the target of the click isn't the container...
            && container.has(e.target).length === 0) {
            container.hide();
        }
        ...
        function(isConfirm) {
            ...
            if (isConfirm) {							  
                ...
                if( subcribed_staff_id > 0 ){
                    ...
                    jQuery.ajax({
                        type: "POST",
                        url: "/reports/complaints_remove_temp_managers",
                        data: {
    ...
    function get_managers_complaints() {
        jQuery.ajax({
            type: "POST",
            url: "/reports/get_managers_complaints",
            data: {
                id: 1
            }
        }).done(function (ret) {
            
            document.getElementById("subscribed_managers_div").innerHTML = ret;                   
    ...
    function get_agency_complaints() {
        jQuery.ajax({
            type: "POST",
            url: "/reports/get_agency_complaints",
            data: {
                id: 1
            }
        }).done(function (ret) {
            
            document.getElementById("subscribed_agency_div").innerHTML = ret;                   
