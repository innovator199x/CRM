				  ...
				  <img src="/images/logo_login.png" class="company_logo sats_logo" />
				  	...
				  	if (isset($propDet)) {
			  		 ...
			  		 ?>
						...
						<div class="row">
							<div class="col-lg-12">
								<small class="text-muted">*This CRM property is not yet linked to any Ourtradie property, kindly link to a Ourtradie property below.</small>
							</div>
						</div>
			  		<?php
				  	} else { ?>
					...
					?>
						...
						?>
							...
							<table id="pmeProp" class="display table table-striped table-bordered table-hover" >
								...
								<tbody>
								<?php 
								foreach($otList as $row){
									//print_r($row);
								?>
									<tr>
										<td class="api_address_tenant_det_btn"><?php echo str_replace(',','',$row['Address1'].' '.$row['Address2'])?></td>
										<td><?php echo $row['Suburb']; ?></td>
										<td><?php echo $row['State']; ?></td>
										<td><?php echo $row['Postcode']; ?></td>
										<td><a class="btn_link" href="javascript:void(0)" data-toggle="tooltip" title="Link to this property" dat-id="<?php echo $row['ID']; ?>">
											<i class="fa fa-chain fa-lg"></i></a>

											<input type="hidden" class="ot_tenant_list_hidden_table" value='<?php echo json_encode($row['tenants']) ?>' />
										</td>
									</tr>			
								<?php
                                }
...
jQuery(document).ready(function() {
	...
	var crm_full_address = jQuery(".crm_full_address").val().trim();
	//console.log(crm_full_address);
	...
	$(document).on('click', '.btn_link', function() {
		var otId = $(this).attr('dat-id');
		var crmId = $("#add0").val();
		swal({
			title: "Are you sure?",
			text: "This will link this Ourtradie property to the CRM property above.",
			type: "warning",
		...
		function(isConfirm) {
			...
			$('#load-screen').show(); 
			$.ajax({
				url: "/ourtradie/ajax_function_link_property",
				type: 'POST',
				data: { 
					'otId': otId,
					'crmId': crmId
