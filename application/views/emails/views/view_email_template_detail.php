...
// breadcrumbs template
    $bc_items = array(
        ...
        array(
            'title' => $title,
            'status' => 'active',
            'link' => "/email/view_email_template_detail/?id=" . $template_id
        )
    );
                    ...
                    <div class="row">
                        <div class="col-md-5">
                            <div class="addproperty">
                                <form method="POST" id="template_form" action="/email/view_update_template_action_form_submit">
                                    <div class="row">
                                        <label class="addlabel">Template Name</label>
                                        <input required="required" value="<?Php echo $template_detail['template_name'] ?>" type="text" class="form-control addinput template_name" name="template_name" id="template_name" <?php echo ($edit_permission == false ? 'disabled' : ''); ?> />
                                        <input type="hidden" name="template_id" value="<?Php echo $template_id ?>">
                                    </div>

                                    <div class="row">
                                        <label class="addlabel">Subject</label>
                                        <input required="required" value="<?Php echo $template_detail['subject'] ?>" type="text" class="form-control addinput subject" name="subject" id="subject" <?php echo ($edit_permission == false ? 'disabled' : ''); ?> />
                                    </div>

                                    <div class="row">
                                        <label class="addlabel">Type</label>
                                        <select required="required" id="temp_type" name="temp_type" class="form-control addinput temp_type" <?php echo ($edit_permission == false ? 'disabled' : ''); ?>>
                                            <option value="">--- Select ---</option>
                                            <?php
                                            if (count($template_types) > 0) {
                                                foreach ($template_types as $ett) {
                                                    $type = $template_detail['temp_type'];
                                                    $selected = "";
                                                    if ($type === $ett['email_templates_type_id']) {
                                                        $selected = 'selected="selected"';
                                                    }
                                                    ?>																
                                                    <option <?Php echo $selected ?> value="<?php echo $ett['email_templates_type_id'] ?>"><?php echo $ett['name'] ?></option>						
                                                    <?php
                                                }
                                            }
                                            ?>	
                                    ...
                                    <div class="row">
                                        ...
                                        <textarea required="required" name="et_body" id="et_body" class="form-control addtextarea et_body" <?php echo ($edit_permission == false ? 'disabled' : ''); ?>><?Php echo $template_detail['body'] ?></textarea>
                                    </div>

                                    <div class="row">
                                        <label class="addlabel">Call Centre</label>			
                                        <select required="required" name="show_to_call_centre" id="show_to_call_centre" class="form-control addinput" <?php echo ($edit_permission == false ? 'disabled' : ''); ?>>
                                            <option value="">----</option>
                                            <option value="1" <?php echo ( $template_detail['show_to_call_centre'] == 1 ) ? 'selected="selected"' : ''; ?>>Yes</option>
                                            <option value="0" <?php echo ( $template_detail['show_to_call_centre'] == 0 && is_numeric($et['show_to_call_centre']) ) ? 'selected="selected"' : ''; ?>>No</option>
                                        </select>
                                    </div>
                                    <div class="row">
                                        <label class="addlabel">Active</label>			
                                        <select name="active" id="active" class="addinput form-control" <?php echo ($edit_permission == false ? 'disabled' : ''); ?>>

                                            <option value="">----</option>
                                            <option value="1" <?php echo ( (int) $template_detail['et_active'] == 1 ) ? 'selected="selected"' : ''; ?>>Yes</option>
                                            <option value="0" <?php echo ( (int) $template_detail['et_active'] == 0 ) ? 'selected="selected"' : ''; ?>>No</option>
                                        </select>
                                    </div>
...
<div id="about_page_fb" class="fancybox" style="display:none;" >
    ...
    <p>This page is used to view and update email template details.</p>
