...
$bc_items = array(
    ...
    array(
        'title' => $title,
        'status' => 'active',
        'link' => "/agency/view_deactivated_agencies"
    )
);
...
$export_links_params_arr = array(
    ...
    'sales_rep_filter' => $this->input->get_post('salesrep_filter'),
    'sub_region_ms' => $this->input->get_post('sub_region_ms'),
    'search_filter' => $this->input->get_post('agency_using_filter'),
    'search_filter' => $this->input->get_post('search'),
    'date_from' => $this->input->get_post('date_from'),
    'date_to' => $this->input->get_post('date_to'),
);
$export_link_params = '/agency/view_deactivated_agencies/?export=1&'.http_build_query($export_links_params_arr);
?>
	...
	<header class="box-typical-header">
        ...
        echo form_open('/agency/view_deactivated_agencies',$form_attr);
        ?>
                ...
                <div class="col-lg-10 col-md-12 columns">
                    <div class="row">
                       ...
                       <div class="col-mdd-3">
                        ...
                        <div class="col-mdd-3">
                           <label>Sales Rep</label>
                            <select id="salesrep_filter" name="salesrep_filter" class="form-control">
                                <option value="">ALL</option>
                                <?php 
                                    foreach($sales_rep_filter->result_array() as $row){
                                        $selected = ($this->input->get_post('salesrep_filter')==$row['salesrep'])?'selected="true"':NULL;
                                ?>
                                        <option <?php echo $selected; ?> value="<?php echo $row['salesrep'] ?>"><?php echo $row['FirstName']." ".$row['LastName'] ?></option>
                                <?php
                                    }
                                ?>
                            </select>
                       ...
                       <div class="col-mdd-3">
                        ...
                        <div class="col-mdd-3">
                           <label>Using</label>
                            <select id="agency_using_filter" name="agency_using_filter" class="form-control">
                                <option value="">ALL</option>
                                <?php 
                                    foreach($using_filter->result_array() as $row){
                                        $selected = ($this->input->get_post('agency_using_filter')==$row['agency_using_id'])?'selected="true"':NULL;
                                ?>
                                        <option <?php echo $selected; ?> value="<?php echo $row['agency_using_id'] ?>"><?php echo $row['name'] ?></option>
                                <?php
                                    }
                                ?>
                            </select>
                        ...
                        <div class="col-mdd-3">
                            ...
                            <select id="status_filter" name="status_filter" class="form-control">
                                ...
                                <option value="deactivated" <?php echo ( $status_filter == 'deactivated' )?'selected':null; ?>>Deactivated</option>
                                <option value="target" <?php echo ( $status_filter == 'target' )?'selected':null; ?>>Target</option>
                            </select>
                        </div>

                        <div class="col-mdd-3">
                            <label>From</label>
                            <input name="date_from" id="date_from" class="flatpickr form-control" data-allow-input="true" id="flatpickr" type="text" value="<?php echo $this->input->get_post('date_from'); ?>" />
                        </div>

                        <div class="col-mdd-3">
                            <label>To</label>
                            <input name="date_to" id="date_to" class="flatpickr form-control" data-allow-input="true" id="flatpickr" type="text" value="<?php echo $this->input->get_post('date_to'); ?>" />
                        </div>
				...
				<table class="table table-hover main-table">
                    ...
                    ?>
                        <tr>
							...
							<td><?php echo ( $row['postcode_region_id']!="" )?$row['postcode_region_name']:''; ?></td>
							<td><?php echo $this->system_model->isDateNotEmpty($row['joined_sats']) ? $this->system_model->formatDate($row['joined_sats'],'d/m/y') : NULL;  ?></td>
                            <td><?php echo ($row['deactivated_ts']===NULL)?NULL:$this->system_model->formatDate($row['deactivated_ts'],'d/m/Y')  ?></td>
                            <td><?php echo $row['deactivated_reason'] ?></td>
                            <td><?php echo $row['active_prop_with_sats'] ?></td>
                            <td><?php echo $row['agency_using'] ?></td>
                        </tr>
                    <?php
                        }
                    ?>
...
<div id="about_page_fb" class="fancybox" style="display:none;" >
	...
	<p>
	This page displays all Agencies who have been deactivated.
	</p>
    <pre>
    <code><?=$sql_query?></code>
    </pre>
